#!/usr/bin/env python3
"""Build the Gemma 2 9B 4k fast-read artifact from Modal reports."""
# ruff: noqa: E402
...
def main() -> None:
    ...
    sentence = _row_from_report(
        "gemma2-9b-4k-sentence-1024-guarded.json",
    ...
    payload = {
        ...
        "provenance": {
            ...
            "notes": (
                "Gemma 2 9B fast-read artifact. Published rows embed per-fixture "
        ...
        "run_conditions": {
            ...
            "sentence_top_k": 3,
            "note": (
                "Same-lab follow-up to Gemma 4. Three-row fast-read for the disambiguation "
                "question: vanilla, TOVA@1024, and sentence_vorn@1024 on the standard 4k NIAH slice."
            ),
        },
    }
    _json_write(RESULTS / "gemma2-9b-4k-fast-read-2026-05-16.json", payload)
    ...
    _md_write(
        RESULTS / "gemma2-9b-4k-fast-read-2026-05-16.md",
        f"""
