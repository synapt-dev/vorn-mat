#!/usr/bin/env python3
"""Build the Gemma 2 9B sentence-attention extension artifact."""
# ruff: noqa: E402
...
def main() -> None:
    historical_vanilla = _row_from_report(
        _load_historical("gemma2-9b-4k-vanilla.json"),
        label="vanilla_full_context_historical",
        method="vanilla",
        budget=None,
        guardrails="prefix_plus_recent",
        ceiling_status="historical_stale_reference",
        source_report=".benchmarks/cross-model/gemma2-9b-4k-vanilla.json",
    )
    historical_sentence = _row_from_report(
        _load_historical("gemma2-9b-4k-sentence-1024-guarded.json"),
        label="sentence_1024_guarded_historical",
        method="sentence_vorn",
        budget=1024,
        guardrails="prefix_plus_recent",
        ceiling_status="historical_stale_reference",
        source_report=".benchmarks/cross-model/gemma2-9b-4k-sentence-1024-guarded.json",
    )
    historical_tova = _row_from_report(
        _load_historical("gemma2-9b-4k-tova-1024.json"),
        label="tova_1024_guarded_historical",
        method="tova_style",
        budget=1024,
        guardrails="prefix_plus_recent",
        ceiling_status="historical_stale_reference",
        source_report=".benchmarks/cross-model/gemma2-9b-4k-tova-1024.json",
    )

    sentence_tova_rows = []
    for budget in (256, 512, 1024):
        filename = f"gemma2-9b-4k-sentence-tova-b{budget}-n50-report.json"
    ...
    payload = {
        ...
        "coverage_notes": [
            "This extension follows the narrowed 2026-05-20 Phase 2B dispatch: it adds sentence-TOVA-style rows across 256, 512, and 1024 for Gemma 2 9B.",
    ...
    _json_write(RESULTS / "sentence-attention-2x2-gemma2-9b-2026-05-20.json", payload)
    ...
    _md_write(
        RESULTS / "sentence-attention-2x2-gemma2-9b-2026-05-20.md",
        f"""
