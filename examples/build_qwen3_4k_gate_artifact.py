#!/usr/bin/env python3
"""Build the Qwen 3 4k cross-model gate artifact from Modal reports."""
# ruff: noqa: E402
...
def _smoke_probe(report: dict[str, object], *, label: str, cases: int, max_new_tokens: int) -> dict[str, object]:
    metric_name, hit_rate = _primary_metric(report)
    observations = _observations(report)
    hits = sum(1 for observation in observations if observation.correct)
    return {
...
def main() -> None:
    floor = _row_from_report(
        _load_report("qwen3-4k-vanilla.json"),
        label="vanilla_floor",
        method="vanilla",
        budget=None,
    ...
    token = _row_from_report(
        _load_report("qwen3-4k-token-1024.json"),
    ...
    sentence_guarded = _row_from_report(
        _load_report("qwen3-4k-sentence-1024-guarded.json"),
    ...
    sentence_noguards = _row_from_report(
        _load_report("qwen3-4k-sentence-1024-noguards.json"),
        label="sentence_1024_noguards",
        method="sentence_vorn",
        budget=1024,
        guardrails="none",
    )
    vanilla_smoke_128 = _smoke_probe(
        _load_report("qwen3-4k-vanilla-128tok-smoke.json"),
        label="vanilla_smoke_128tok",
        cases=5,
        max_new_tokens=128,
    )
    ...
    payload = {
        ...
        "auxiliary_probes": [vanilla_smoke_128],
        "run_conditions": {
            "profile": "author",
    ...
    _json_write(RESULTS / "qwen3-4k-gate-2026-05-15.json", payload)
    ...
    _md_write(
        RESULTS / "qwen3-4k-gate-2026-05-15.md",
        f"""
...
- `vanilla_smoke_128tok`: `5` cases at `max_new_tokens=128` reached `hit_rate = {vanilla_smoke_128['hit_rate']:.2f}` (`{vanilla_smoke_128['hits']}/{vanilla_smoke_128['cases']}`), compared with the main `32`-token vanilla gate row at `{floor['hit_rate']:.2f}`.
...
- Under the current `32`-token answer-budget surface, Qwen 3 is a flat-zero model on this slice. Vanilla full-context performance is `{floor['hit_rate']:.2f}`, so the fixed-budget comparison rows do not show usable recovery at `1024`.
