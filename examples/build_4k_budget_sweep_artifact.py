#!/usr/bin/env python3
"""Build the 4k token-vs-sentence budget sweep artifact from Modal reports."""
# ruff: noqa: E402
...
def _load_4k_ceiling() -> dict[str, object]:
    payload = json.loads((RESULTS / "sentence-level-eviction-2026-05-13.json").read_text())
...
def main() -> None:
    ceiling = _load_4k_ceiling()

    token_rows = [
        _row_from_live_report(
            _load_report("4k-token-vorn-b512-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-token-vorn-b1536-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-token-vorn-b2048-report.json"),
    ...
    sentence_rows = [
        _row_from_live_report(
            _load_report("4k-sentence-vorn-b512-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-sentence-vorn-b512-noguards-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-sentence-vorn-b1536-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-sentence-vorn-b1536-noguards-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-sentence-vorn-b2048-report.json"),
        ...
        _row_from_live_report(
            _load_report("4k-sentence-vorn-b2048-noguards-report.json"),
    ...
    _json_write(RESULTS / "sentence-level-eviction-4k-budget-sweep-2026-05-13.json", payload)
    ...
    _md_write(
        RESULTS / "sentence-level-eviction-4k-budget-sweep-2026-05-13.md",
        f"""
...
- The 4k curve is not the 8k U-shape shifted left. Sentence-level is roughly tied with token-level at `512` (`0.26` vs `0.20`, paired `p = {pairwise_tests[0]['p_value']:.6g}`), peaks sharply at `1024` (`0.68`), then decays through `1536` (`0.54`) and `2048` (`0.36`) while still beating token-level at each of those higher budgets.
- Guardrail-independence is again budget-dependent. At `512`, removing guardrails collapses sentence-level to `0.02` (`p = {pairwise_tests[1]['p_value']:.6g}`); at `1024`, `1536`, and `2048`, guarded and no-guardrails sentence rows are identical (`p = 1`).
