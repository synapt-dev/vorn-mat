#!/usr/bin/env python3
"""Build the Gemma 4k cross-model gate artifact from Modal reports."""
# ruff: noqa: E402
...
def main() -> None:
    floor = _row_from_report(
        _load_report("gemma-4-4k-vanilla.json"),
        label="vanilla_floor",
        method="vanilla",
        budget=None,
    ...
    token = _row_from_report(
        _load_report("gemma-4-4k-token-1024.json"),
    ...
    sentence_guarded = _row_from_report(
        _load_report("gemma-4-4k-sentence-1024-guarded.json"),
    ...
    sentence_noguards = _row_from_report(
        _load_report("gemma-4-4k-sentence-1024-noguards.json"),
    ...
    _json_write(RESULTS / "gemma-4-4k-gate-2026-05-15.json", payload)
    ...
    _md_write(
        RESULTS / "gemma-4-4k-gate-2026-05-15.md",
        f"""
...
- Gemma is not a Qwen-style drowning-floor model on this slice. With the narrowed numeric-span NIAH compatibility rule, vanilla full-context performance is a clean `{floor['hit_rate']:.2f}`.
- At the same `1024` budget and nearly identical retention ratios, sentence-vorn preserves materially more quality than token-vorn: `{sentence_guarded['hit_rate']:.2f}` vs `{token['hit_rate']:.2f}`, exact McNemar `p = {pairwise_tests[0]['p_value']:.6g}`.
- Guardrails do not explain the sentence result here. Sentence no-guardrails stays essentially flat at `{sentence_noguards['hit_rate']:.2f}` against guarded `{sentence_guarded['hit_rate']:.2f}`.
