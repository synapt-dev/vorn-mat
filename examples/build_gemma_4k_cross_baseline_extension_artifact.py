#!/usr/bin/env python3
"""Build the Gemma 4k cross-baseline extension artifact."""
# ruff: noqa: E402
...
def main() -> None:
    ...
    tova = _row_from_report(
        _load_report("gemma-4-4k-tova-1024.json"),
    ...
    h2o = _row_from_report(
        _load_report("gemma-4-4k-h2o-1024.json"),
    ...
    payload = {
        ...
        "run_conditions": {
            ...
            "note": (
                "Extends the original Gemma 4 gate with TOVA- and H2O-style attention baselines "
                "on the same slice, budget, and paired-observation surface."
            ),
        },
    }
    _json_write(RESULTS / "gemma-4-4k-cross-baseline-extension-2026-05-15.json", payload)
    ...
    _md_write(
        RESULTS / "gemma-4-4k-cross-baseline-extension-2026-05-15.md",
        f"""
...
- `TOVA` and `H2O` are effectively equivalent on quality here (`0.94` vs `0.94`; exact McNemar `p = 1.0`), but not on cost. `H2O` took `{h2o['elapsed_seconds']:.2f}s` and `${h2o['estimated_cost_usd']:.4f}` versus `TOVA` at `{tova['elapsed_seconds']:.2f}s` and `${tova['estimated_cost_usd']:.4f}`.
