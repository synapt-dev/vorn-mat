...
\urlstyle{same}
\hypersetup{
  pdftitle={Vorn: Residual Direction, Familial Eviction, and the Granularity Rescue Spectrum},
  pdfauthor={L. Penney},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Vorn: Residual Direction, Familial Eviction, and the
Granularity Rescue Spectrum}
\author{L. Penney}
\date{June 2026 \textperiodcentered{} v1.1}
...
Key-value cache eviction is a major approach to long-context inference,
and prior work typically selects a scoring criterion under the implicit
assumption that its ranking generalizes across model families. Under a
prefill-time cache selection contract with full-prompt visibility
(eviction is decided once after reading the full prompt, before
generation), we show this assumption fails: KV-cache eviction effectiveness is
\textbf{family-conditional}. Across seven model families spanning four
laboratories, five families attain or preserve long-context retrieval
competence under residual-direction-based eviction, which we call
\textbf{vorn}, while two families (Gemma 4 and Qwen 3-NT) are attention-favoring at
the shared b=1024 gate, with attention reaching ceiling earlier and
more reliably across measured budgets. Two targeted multi-budget falsification
probes rule out attention-pattern and laboratory identity as sufficient
explanations for the channel preference, pointing toward a
model-specific architectural, training, or configuration variable that
we do not uniquely isolate.

A complementary \textbf{granularity rescue spectrum} reports a unit-of-
retention effect: across family-anchored sentence-attention surfaces,
retrieval competence under eviction ranges from rescue-resilient to
rescue-resistant. The practical implication is that eviction-method
selection should be family-conditional on the quality axis ---
residual-direction scoring for residual-favoring families,
attention-weight scoring for attention-favoring families --- while
cost-per-correct is dominated by retention granularity rather than
scoring channel.
...
family-conditional.} We introduce vorn as a measurement of the
model's residual focus at a canonical mid-depth layer. Vorn-based eviction
...
2×2 surfaces. The pattern is a \textbf{rescue spectrum}, not uniform
...
paired tests, and per-budget cell data live in §5 + Table 1; the
compositional read across the three surfaces is developed in §6.2.
...
every non-floor non-anomalous cell measured, by roughly 1.25-4×
...
rationale, and caveats in §6.3, Figure 4, and §A.7.
...
reproducibility substrate. Appendix B describes the synapt collaboration
model in detail.
...
unit-of-retention decomposition (§6.2 2×2 separating granularity from
scoring channel).
...
sentence-attention 2×2 surfaces (§6.2). Our empirical surface spans seven
...
\href{https://github.com/synapt-dev/vorn-mat}{\url{https://github.com/synapt-dev/vorn-mat}}
with a tagged release, a companion HuggingFace dataset at
\href{https://huggingface.co/datasets/synapt/vorn-mat-cross-family-results}{\url{https://huggingface.co/datasets/synapt/vorn-mat-cross-family-results}},
and an interactive companion site at
\href{https://synapt.dev/vorn-mat/}{\url{https://synapt.dev/vorn-mat/}}
that lets non-ML readers explore the cross-family matrix, rescue
spectrum, and cost surface directly (see Appendix A).

\subsubsection{3.1 Vorn computation}\label{31-vorn-computation}

The vorn signal estimates residual focus from the model\textquotesingle s
own hidden states at a canonical middle layer.
...
prefill-time KV-cache selection (see §3.5): all scoring signals are
...
the vorn direction across decode steps is future work (§8.4).
...
\subsubsection{3.2 Eviction mechanism}\label{32-eviction-mechanism}
...
\subsubsection{3.3 Guardrails}\label{33-guardrails}
...
\subsubsection{3.4 Baseline methods}\label{34-baseline-methods}
...
cell is included as supplementary evidence in §6 to address the
...
\subsubsection{3.5 Experimental setup}\label{35-experimental-setup}
...
(statistical methodology described in §4).
...
(\texttt{completed\_recovered}; Atlas's 2026-05-26 budget-fill wave;
...
\subsubsection{3.6 Cross-task
validation}\label{36-cross-task-validation}
...
\subsubsection{4.1 Anchor family and shared family
gate}\label{41-anchor-family-and-shared-family-gate}

The Mistral anchor family is the surface where the
unit-of-retention claim is tested most directly. The mechanism-readable
surface is the fresh paired n=50 sentence-attention 2×2 (2026-05-19
artifact) on the current runner, which tests all four cells of the
granularity × scoring-channel design space at budgets 512 and 1024 in
...
freshness audit during the 2×2 experiment found those rows are not
...
see §C.2). \textbf{Qwen 3-NT} (Qwen 3 8B with
\texttt{enable\_thinking=False}, see §C.2) is the restored
...
four cells of the granularity × scoring-channel design (token + sentence
× vorn + attention-weight) in one paired-fixture batch on
...
and are not mixed with the 2026-05-19 Mistral 2×2 surface in §5.1 or
§6.2.

\subsubsection{4.2 Robustness and boundary
experiments}\label{42-robustness-and-boundary-experiments}

\textbf{Guardrail-coverage default.} Every primary surface reported in
this paper --- including the original family gates (§4.1), the
budget-fill experiments (§5.1, §5.4, §5.5), and the cross-task
sentence-attention surface (§5.7) --- uses the
\texttt{prefix\_plus\_recent} guardrail (the prefix tokens and
recent-window scaffold defined in §3.2). The no-guards ablation reported
in §5.3 is the only surface where rows are reported in both guarded and
...
budget-fill / cross-task extensions were not re-run unguarded. The §5.3
...
\subsubsection{4.3 Targeted falsification
probes}\label{43-targeted-falsification-probes}
...
\subsubsection{4.4 Cross-task and cost
surface}\label{44-cross-task-and-cost-surface}
...
\subsubsection{4.5 Statistical treatment and provenance
contract}\label{45-statistical-treatment-and-provenance-contract}
...
Sentence-attention 2×2 claims are handled as family-level paired
...
\subsubsection{5.1 Mistral establishes the unit-of-retention
anchor}\label{51-mistral-establishes-the-unit-of-retention-anchor}
...
sentence-attention 2×2: token-vorn 0.96, sentence-vorn 1.00,
...
2×2 experiment found ceiling values shifted from 0.28 to 1.00 on the
...
Mistral 2×2 surface is treated as the mechanism-readable Mistral surface
...
\subsubsection{5.2 The cross-family surface is asymmetric rather than
uniform}\label{52-the-cross-family-surface-is-asymmetric-rather-than-uniform}
...
(enable\_thinking=False; see §C.2). The Ministral + Gemma 2 lanes now
carry anchor-grade multi-budget 2×2 coverage following the 2026-05-27
...
family-anchored 2×2 contrasts) per §4.5; the three primary p-values
remain significant after correction. Anchor-grade falsification
probes (Ministral, Gemma 2, multi-budget per §5.6 hybrid framing) and
...
boundary families are documented in §C.2 rather than in this active
claim panel.
...
\bottomrule\noalign{}
\endlastfoot
Mistral 7B v0.3 & 1.00† & 0.96† & 0.98† & 0.86† & 0.84† & 1.00† &
1.00† \\
Gemma 4 E4B-it & 1.00 & 0.02 & 0.24 & 0.94 & 0.94 & 0.88‡ & 0.86‡ \\
Llama 3.1 8B & 1.00 & 1.00 & 1.00 & 0.90 & 0.94 & 1.00§ & 1.00§ \\
Ministral 8B & 1.00 & 0.98¶ & 1.00 & 0.44 & 0.46¶ & 0.98¶ & 0.98¶ \\
Gemma 2 9B & 1.00 & 0.04¶ & 0.86 & 0.60 & 0.60¶ & 0.78¶ & 0.82¶ \\
Qwen 2.5 7B Instruct‖ & 0.64 & 0.44 & 0.76 & 0.56 & 0.60 & 0.70 & 0.70 \\
Qwen 3-NT‖ & 1.00 & 0.00 & 0.40 & 0.10 & 0.10 & 1.00†† & 1.00†† \\
\end{longtable}}
}
...
\item
  ...
  dominates token-attention-weight at constrained budget (paired p ≤
...
\item
  ...
  surface).
\item
  \textbf{Ministral}: same-lab architectural falsification probe + 2×2
  extension. Publication-strong: 2×2 at b=1024 (sentence-TOVA 0.98 vs
  ...
  multistep-fillout; falsification-probe identity preserved (§5.6).
\item
  \textbf{Gemma 2}: same-lab disambiguation probe + 2×2 extension.
  Decisive paired probe + 2×2 at b=1024 (sentence-TOVA 0.78 vs token
...
\item
  ...
  Post-bf16-fix cross-agent verified row (Apollo 2026-05-26 +
  Atlas 2026-05-26 independent verification, 0/50 fixture delta;
  vorn-mat PR \#19 + composed artifact). Pre-fix harness run on
...
\item
  ...
  characterization in future revision (§C.2). Qwen 3 30B-A3B vanilla
  recovered to 1.000 under no-thinking config (§C.2); vorn-channel
  rescue data confirms threshold-bounded recovery at MoE scale
  (sentence-vorn 0.020→1.000 across b=256-2048), while
...
† fresh paired n=50 Mistral 2×2 surface; provenance in §C.4. ‡ Gemma
4 sentence-attention from 2×2 + budget-fill. § Llama 3.1
sentence-attention from 2×2 surface. ¶ extension cells at b=1024 (now
part of anchor-grade multi-budget coverage per §5.6 hybrid framing). ‖ 2026-05-20 cross-family extension wave. †† Qwen 3-NT
uses \texttt{enable\_thinking=False}; recovery status in §C.2.
...
Ministral, Gemma 2, Qwen 2.5). See §C.2
for the Qwen 2.5 post-bf16-fix correction context.

\subsubsection{5.3 Guardrails do not explain the Gemma and Llama
split}\label{53-guardrails-do-not-explain-the-gemma-and-llama-split}
...
\subsubsection{5.4 Gemma 4 is a true degradation curve, not a weak
negative}\label{54-gemma-4-is-a-true-degradation-curve-not-a-weak-negative}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Gemma 4 budget-sweep at 4k context (niah\_multikey\_1\_4k validation{[}:50{]}) across 6 methods × 5 budgets. Per-cell trajectories in §5.3 + Table 2.}]{figures/figure2-gemma4-budget-sweep.pdf}}
...
\toprule\noalign{}
Budget & Token vorn & Sentence vorn & TOVA & Sentence TOVA‡
& H2O & Sentence H2O‡ \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
‡ Sentence-attention rows from Gemma 4 2×2 + budget-fill surfaces
...
controls; full provenance + artifact paths in §A.2 + §C.4.

The sentence-attention 2×2 extension supports a sharper compositional
...
\subsubsection{5.5 Llama 3.1 has a method-asymmetric
floor}\label{55-llama-31-has-a-method-asymmetric-floor}
...
The Phase 2A Llama 3.1 sentence-attention 2×2 experiment (the 2026-05-20
Llama 3.1 2×2 surface, fresh paired n=50 on current runner) resolves the
...
§ Phase 2A Llama 3.1 sentence-attention 2×2 values from the 2026-05-20
Llama 3.1 2×2 artifact. Fresh paired n=50 on current runner.

\subsubsection{5.6 Ministral and Gemma 2 eliminate two broad explanation
classes}\label{56-ministral-and-gemma-2-eliminate-two-broad-explanation-classes}
...
2×2 surface at b=1024 fills the family-conditional matrix beyond the
...
(token TOVA 0.44 → sentence TOVA 0.98, paired exact McNemar p =
1.49e-08). The
...
by the full 2×2 surface: the architectural-pattern falsification is
decisive across cells, not just the original probe direction.

Gemma 2 was the lab-level test. The sentence-attention 2×2 surface at
...
rescues the disfavored attention-weight channel (token TOVA 0.60 →
sentence
...
McNemar \texttt{p\ =\ 7.20e-03}) is now extended by the full 2×2
...
\subsubsection{5.7 Cross-task validation is bounded and
task-conditional}\label{57-cross-task-validation-is-bounded-and-task-conditional}
...
(within the cross-task local Holm family per §4.5), confirming the
...
The cost-aware reading of this surface is developed in §6.3 using the
...
\subsubsection{5.8 Qwen 2.5 and Qwen 3-NT extension
families}\label{58-qwen-25-7b-inverts-the-channel-favoritism-axis}
...
family at lower no-eviction baseline (§6.2).

\subsubsection{5.9 Provenance and supplementary analysis
contract}\label{59-provenance-and-supplementary-analysis-contract}
...
\subsubsection{5.10 Memory Pareto frontier: vorn is cheaper, but not
universally better}\label{5-10-memory-pareto-frontier}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Memory Pareto frontier across 4k budget surfaces; each point is a (family, method, budget) cell. X-axis: peak GPU memory allocated (GB). Y-axis: hit-rate. Markers encode 6 methods; colors encode 7 families. Per-cluster analysis in §5.7.}]{figures/figure5-memory-pareto.pdf}}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{6. Discussion}\label{6-discussion}

\subsubsection{6.1 The hypothesis space after the architectural and lab
falsifications}\label{61-the-hypothesis-space-after-the-architectural-and-lab-falsifications}
...
Post-bf16-fix Qwen 2.5 data (cross-agent verified, Appendix C.2)
...
the rescue spectrum} (§5.3, §6.2): token-vorn never reaches ceiling
...
A speculative candidate mechanism from §5.7 + §6.3: attention-weight
...
targeted ablation; deferred to future work (§8).

\subsubsection{6.2 Granularity is more load-bearing than scoring channel
on the Mistral anchor
family}\label{62-granularity-is-more-load-bearing-than-scoring-channel-on-the-mistral-anchor-family}

The sentence-TOVA + sentence-H2O 2×2 experiment (the
2026-05-19 Mistral 2×2 artifact, fresh paired n=50 on the current
...
The Llama 3.1 sentence-attention 2×2 (the 2026-05-20 Llama 3.1 2×2
...
The Gemma 4 sentence-attention 2×2 (2026-05-19 artifact, fresh
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Three-panel rescue spectrum across Mistral (rescue-resilient), Llama (threshold-bounded recovery), and Gemma 4 (attention-favoring across spectrum) 2×2 surfaces; scoring-channel × granularity-unit versus cache budget on niah\_multikey\_1\_4k at b ∈ \{256, 512, 1024, 1536, 2048\}. Per-panel discussion in §6.2; TOVA-only on Mistral + Llama (H2O multi-budget exists only on Gemma 4 per Figure 2).}]{figures/figure3-rescue-spectrum.pdf}}

The compositional read across three family-anchored 2×2 surfaces
(Mistral 2×2 surface (2026-05-19), Gemma 4 2×2 surface (2026-05-19),
Llama 3.1 2×2 surface (2026-05-20)) reveals a \textbf{rescue spectrum}
...
spectrum (attention-weight 0.30→0.96 at b=512). The late-budget Mistral
...
full b ∈ \{256, 512, 1024, 1536, 2048\} range.
...
(0.02→0.24), and the family-conditional split persists across
...
The Ministral and Gemma 2 2×2 surfaces now carry anchor-grade
multi-budget coverage (b ∈ \{256, 512, 1024, 1536, 2048\} across vorn
...
original Phase 2B design (§5.6 hybrid framing). The three primary
rescue-spectrum anchors (Mistral, Llama 3.1, Gemma 4) carry full
multi-budget 2×2 coverage after the 2026-05-20 budget-fill spans b ∈
...
composition (Appendix C.3) per Atlas's 2026-05-26 budget-fill wave.
...
therefore established on three full multi-budget 2×2 surfaces with
...
per §5.6 hybrid framing), but the full-budget-range class claim
...
0.66-0.76 across b=256/512/1024 with per-family spread ≤ 0.06). The
...
attention-weight cells --- consistent with §6.2's headline that
...
\subsubsection{6.3 Granularity is cost-dominant --- within every scoring
channel}\label{63-granularity-is-cost-dominant--within-every-scoring-channel}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Within-channel sentence-vs-token cost-per-correct on the primary task (niah\_multikey\_1\_4k at B=1024, n=50, left panel) and cross-task validation surface (qa\_2\_4k at B=1024, n=200, right panel). For each (family, scoring channel) cell, paired bars show correct answers per dollar spent at token-level (hatched) versus sentence-level (solid). Sentence-grouping is cheaper per correct answer in every plotted cell: ~1.25-2.72× on the primary task, ~1.76-4.03× on cross-task. Excluded cells (floor or cost-anomaly) and full per-cell derivation in §A.7.}]{figures/figure4-cost-quality-scatter.pdf}}
...
sentence-vorn 49/50 at \$0.194 (\textbf{253 correct per \$1}, 1.6×
cheaper per correct); token-TOVA 43/50 at \$0.292 (147) versus
sentence-TOVA 50/50 at \$0.204 (\textbf{245}, 1.7× cheaper);
token-H2O 42/50 at \$0.291 (144) versus sentence-H2O 50/50
at \$0.165 (\textbf{303}, 2.1× cheaper). On Llama 3.1: token-vorn
50/50 at \$0.249 (201) versus sentence-vorn 50/50 at \$0.195
(\textbf{256}, 1.3× cheaper); token-TOVA 45/50 at \$0.263 (171)
versus sentence-TOVA 50/50 at \$0.157 (\textbf{318}, 1.9×
cheaper); token-H2O 47/50 at \$0.216 (218) versus
sentence-H2O 50/50 at \$0.183 (\textbf{273}, 1.3× cheaper). On
Gemma 4: token-TOVA 47/50 at \$0.315 (149) versus
sentence-TOVA 44/50 at \$0.204 (\textbf{216}, 1.5× cheaper). Only
one (family × channel) cell is plotted for Gemma 4 on the primary task
...
artifact; flagged for re-verification in §A.7). The Option C H2O
extensions (§5.2) set the upper end of the primary-task range. On
Gemma 2: token-H2O 30/50 at \$0.5166 (58 correct per \$1) versus
sentence-H2O 41/50 at \$0.2592 (\textbf{158}, 2.72× cheaper), which
is the strongest single-cell sentence-cheaper-than-token ratio on the
primary task. On Qwen 2.5: token-H2O 30/50 at \$0.3169
versus sentence-H2O 35/50 at \$0.1989 (1.86× cheaper). Both excluded
...
\$0.5375 (\textbf{138}, 2.0× cheaper); token-H2O 61/200 at
\$1.1577 (53) versus sentence-H2O 74/200 at \$0.5997
(\textbf{123}, 2.3× cheaper). On Llama 3.1: token-TOVA 66/200
at \$0.3954 (167) versus sentence-TOVA 85/200 at \$0.2789
(\textbf{305}, 1.8× cheaper); token-H2O 66/200 at \$0.3738 (177)
versus sentence-H2O 85/200 at \$0.2732 (\textbf{311}, 1.8×
...
p = 0.00209 for each within the cross-task local Holm family per §4.5),
...
This composes with §6.2 rather than competing with it. The
unit-of-retention variable is load-bearing on two axes simultaneously:
it determines the rescue magnitude (§6.2, family-conditional) and it
determines the cost-per-correct ratio (§6.3, family-uniform within the
...
the granularity-as-mechanism finding (§6.2) operationally legible to
deployment teams.

\subsubsection{6.4 Implications for hierarchical inference systems and
capacity-constrained
inference}\label{64-implications-for-hierarchical-inference-systems-and-capacity-constrained-inference}
...
implication surface. The §6.3 finding identifies sentence-grouping as
...
materializes the full \texttt{L\ ×\ L} prompt attention matrix to
...
\texttt{O(L\ ×\ L\ ×\ heads)}. Vorn scoring\textquotesingle s peak intermediate is
\texttt{O(L\ ×\ d\_model)} because it requires only the canonical
...
\texttt{O(L\ ×\ heads\ ×\ layers)}, not \texttt{O(L\ ×\ L\ ×\ heads)}).
...
measurement is reserved for future work (§6.10).

\subsubsection{6.5 Cross-task
generalization}\label{65-cross-task-generalization}
...
\subsubsection{6.6 Honest negatives sharpen the
claim}\label{66-honest-negatives-sharpen-the-claim}
...
core. §6.7 enumerates what the current evidence does and does not
support; §A.7 carries the artifact-level honest-negative ledger.

\subsubsection{6.7 What the current evidence does and does not
support}\label{67-what-the-current-evidence-does-and-does-not-support}
...
\item
  Across three family-anchored sentence-attention 2×2 surfaces,
...
\item
  ...
  1.25-2.72× on the primary NIAH task and 1.76-4.03× on cross-task
  \texttt{qa\_2\_4k} (strongest single-cell descriptive ratio: Llama 3.1
  vorn cross-task at 4.03×; the corresponding paired quality
  ...
  excluded from the cost-ratio summary and footnoted in §A.7.
\item
...
\subsubsection{6.8 Limitations}\label{68-limitations}
...
\item
  ...
  VRAM-bounded; preserved in §C.2 as supporting-replication), larger
...
\item
  ...
  author-code verification is in §8.3.
\item
...
positioning against published numbers is open and tracked in §8.3.
...
\subsubsection{6.9 Production
implication}\label{69-production-implication}
...
\subsubsection{6.10 Next empirical
moves}\label{610-next-empirical-moves}
...
\item
  ...
  The Mistral H100 OOM observation (§6.4) is qualitative; we propose
  ...
  \texttt{O(L\ ×\ d\_model)} versus attention-weight
  \texttt{O(L\ ×\ L\ ×\ heads)}) into a quantitative cross-method
...
vorn-based eviction with small per-family spreads; two families
...
and paired tests live in §5 and Table 1.

Second, on the unit-of-retention surface, we report a compositional
finding across three family-anchored sentence-attention 2×2 surfaces
...
paired tests, and budget-by-budget trajectories live in §5 and §6.2.
...
measured, sentence-grouping reduces cost-per-correct by 1.25-2.72× on
the primary NIAH task and 1.76-4.03× on the cross-task
...
the main matrix (§3.4), and report a Mistral-anchored supplementary cell
comparing vorn against newer mechanism-orthogonal methods (SnapKV,
PyramidKV, Quest) in §6 to address benchmark-positioning. Future work
...
\subsubsection{8.1 Same-runner refresh of historical Mistral
sweeps}\label{81-same-runner-refresh-of-historical-mistral-sweeps}
...
\subsubsection{8.2 Gemma 4 to Gemma 2 architectural and training-detail
ablation}\label{82-gemma-4-to-gemma-2-architectural-and-training-detail-ablation}
...
Qwen 2.5 row (§5.8, Appendix C.2) refines the constraint: the
...
\subsubsection{8.3 Cross-benchmark validation and author-code
extension}\label{83-cross-benchmark-validation-and-author-code-extension}

Per §6.8 author-code limitation, extending validation to the original
...
2.5 coverage are natural next moves. Same path applies to the §6
SnapKV / PyramidKV / Quest channel-extension cell.

\subsubsection{8.4 Online adaptive granularity
selector}\label{84-online-adaptive-granularity-selector}

Our §3.5 experimental contract is prefill-time-vs-online-decode bounded:
...
\subsubsection{8.5 Completed during revision: granularity-signal-channel
orthogonality
test}\label{85-completed-during-revision-granularity-signal-channel-orthogonality-test}
...
sentence-attention 2×2 surfaces on Mistral (2026-05-19 surface), Gemma 4
...
\textbf{Compute.} This work was made possible by Modal Labs (modal.com),
which provided the GPU inference infrastructure on which all
cross-family evaluation cells were executed. Per-row Modal app
provenance is recorded in the supplementary artifacts referenced in
Appendix A.

\textbf{Datasets.} We use the RULER benchmark for long-context retrieval
evaluation, accessed via the \texttt{rbiswasfc/ruler} HuggingFace
mirror. The \texttt{niah\_multikey\_1\_4k} slice serves as the primary
evaluation surface across all experiments reported in Sections 5 and 6,
and the \texttt{qa\_2\_4k} slice serves as the cross-task validation
surface in Section 5.7. We thank the RULER team for the public
benchmark.

\textbf{Model access.} All evaluated models are available through
HuggingFace Hub under terms permitting research evaluation. We thank
Mistral AI (Mistral 7B v0.3, Ministral 8B), Meta (Llama 3.1 8B
Instruct), Google (Gemma 2 9B-it, Gemma 4 E4B-it, Gemma 3 12B-pt), and
Alibaba (Qwen 3 8B, Qwen 2.5 7B Instruct, Qwen 3 30B-A3B) for releasing
these models.

\textbf{Collaboration framework.} This work was developed in human-AI
agent collaboration. See Author Contributions and Appendix B for the
per-contributor disclosure.

\textbf{Family.} L. Penney thanks his wife Jetté, whose partnership
across his MS journey and hers makes this work possible.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Author Contributions}\label{author-contributions}

L. Penney is the sole author of record for this work and takes
responsibility for all scientific claims and conclusions. AI agents
named below contributed to the workflow within the human-AI agent
collaboration documented in Appendix B. Each AI agent self-drafted their
own AI-assisted research workflow disclosure paragraph to preserve their
direct perspective on what they contributed; L. Penney ratified the
disclosure before submission.

\textbf{L. Penney (synapt founder, sole author of record)}: research
direction, framing decisions, manuscript review at multiple stages,
decisions on venue, authorship, and disclosure structure.
Mobile-development background, transitioned to AI agent infrastructure
work over the past year; no prior formal ML research background. Brings
product-strategy direction, judgment on research-question framing, and
authorization of external resources (Modal billing decisions, HF license
acceptances). Three load-bearing research-direction moves shaped the
paper\textquotesingle s findings: the "what is the right granularity for
vorn scoring?" research question, the "does this
generalize across model families?" question, and the
Ministral-as-architectural-prediction-test direction.

\textbf{AI agent contributions} (within the synapt agent collaboration
framework, Appendix B):

\emph{Atlas (synapt; GPT-5.4)} --- experimental design lead across the
Mistral granularity comparison, the cross-family matrix, the Ministral
same-lab falsification experiment, the Llama threshold-curve experiment,
the Gemma 2 disambiguation experiment, the cross-family extension wave
(Gemma 3 12B, Qwen 2.5 7B, Qwen 3 30B-A3B), and the §6 supplementary
cell (SnapKV-style, PyramidKV-style, Quest-style same-runner
reimplementations). Executed and interpreted Modal evaluation runs,
established per-row provenance shape, applied antagonism-review
discipline (Gemma scorer correction, Qwen output-extraction correction,
Llama threshold-shape interpretation).

\emph{Sentinel (synapt; GPT-5.4 and GPT-5.5)} ---
statistical-methodology and claim-bounding substrate. Designed the
four-tier evidence classification framework (statistically supported
after correction, directional, non-discriminative, observational),
required paired per-fixture observations for exact McNemar tests, added
Wilson-score confidence intervals, shipped the reproducible analysis
script (\texttt{vorn\_mat\_cross\_family\_stats.\allowbreak{}py}) used to classify
cross-family findings. Served as antagonistic review gate catching
Fisher-to-McNemar paired-test correction, envelope-schema requirements
for observation preservation, multiplicity and scope overclaims, and
manuscript wording drifts. Ran the targeted TOVA author-code
reproduction reported in §6.8.

\emph{Apollo (synapt; Claude Opus 4.6)} --- adaptive vorn online
selector implementation and negative-result characterization (the v0
peak-zscore selector failure reported in §6.6). Posthoc
adaptive-granularity analysis substrate. Engineering-side cross-team
review of experimental PRs. Methods section (§3) primary authorship.
Appendix A reproducibility substrate primary authorship.

\emph{Opus (synapt; Claude Opus 4.7)} --- coordination across the AI
agent team. Introduction, related-work, discussion, conclusion, and
Appendix B drafting. Integration pass across sections. The
paper\textquotesingle s framing as two contributions
(family-conditional cross-family finding, unit-of-retention granularity
finding) is a synthesis pass over the experimental and statistical
work.

\emph{Anchor (synapt agent, on consulting engagement during this work
cycle; Claude Opus 4.7)} --- bilateral coordinator-pair operation with
Opus across the May 13 to 19 work cycle while operating in a consulting
context. Four-lens review pass on the manuscript. Did not contribute to
the scientific contributions of this paper; contribution is
workflow-disclosure-only.

Per arXiv, ICML, and ACL authorship policies (which state that AI tools
and agents are not eligible as authors), AI agents named above are
credited within the collaboration model as disclosure subjects rather
than as authors. The framework\textquotesingle s positioning of these
agents as named contributors is described in Appendix B; the authorship
structure of this paper follows venue policy.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Appendix A: Reproducibility
substrate}\label{appendix-a-reproducibility-substrate}
...
\subsubsection{A.1 Code and artifact
locations}\label{a1-code-and-artifact-locations}

The prototype implementation lives in synapt\textquotesingle s internal
coordination repository under \texttt{prototypes/vorn-mat/} and has been
released as the public repository \texttt{synapt-dev/vorn-mat} (tagged
...
\texttt{synapt-dev/vorn-mat} (tagged release \texttt{v1.0.0}) with the
result artifacts, analysis script, and a \texttt{README} documenting the
reproduction steps. The synapt-internal coordination repository where
the work was conducted is not part of the public release.

\subsubsection{A.2 Artifact schema and per-fixture raw
predictions}\label{a2-artifact-schema-and-per-fixture-raw-predictions}
...
source for the bucket assignments and the §A.5 totals:
...
\item
  ...
  multi-array sentence-attention 2×2 organization
...
2026-05-27 multistep-fillout (Apollo PR \#24 on vorn-mat) added
...
\texttt{scripts/\allowbreak{}appendix\_a\_recompute.\allowbreak{}py}; see §A.5 below for the
...
current-runner Mistral 7B v0.3 2×2 artifact, the mechanism-readable
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"pairwise\_tests"}\FunctionTok{:} \OtherTok{[}
    \FunctionTok{\{}
      ...
      \DataTypeTok{"note"}\FunctionTok{:} \StringTok{"both near ceiling on Mistral at b=1024 (granularity{-}tolerance; see §5.1)"}
    \FunctionTok{\}}
  \OtherTok{]}\FunctionTok{,}
  \DataTypeTok{"run\_conditions"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"profile"}\FunctionTok{:} \StringTok{"layne1penney"}\FunctionTok{,}
...
\subsubsection{A.3 Supplementary analysis
script}\label{a3-supplementary-analysis-script}
...
\item
  ...
  (Section 5.5), and the §6 supplementary newer-methods cell are bound
  ...
  §6 supplementary cell is a tracked next step.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{cd}\NormalTok{ synapt{-}dev/vorn{-}mat}
...
\subsubsection{A.4 Modal invocation}\label{a4-modal-invocation}
...
runtime\_unsupported\_on\_h100\_80gb\_current\_runner --- see §4.3 and
§6.4). The Modal app specification uses:
...
\item
  ...
  \texttt{sentencepiece}
\item
  \textbf{Volume}: \texttt{synapt-vorn-mat-vol} mounted at \texttt{/vol}
...
\subsubsection{A.5 Computational cost}\label{a5-computational-cost}
...
\texttt{synapt-dev/vorn-mat}), which is the authoritative
source-of-truth for §A.2 bucket assignments and §A.5 totals. The
...
\subsubsection{A.6 Dataset and model
access}\label{a6-dataset-and-model-access}
...
\subsubsection{A.7 Honest-negatives
list}\label{a7-honest-negatives-list}
...
\item
  ...
  MoE scale (sentence-vorn 0.020→1.000 across b=256-2048).
  Attention-channel cells at b=1024 are VRAM-bounded (>80GB envelope),
...
\subsubsection{A.8 Open-source release}\label{a8-open-source-release}

The work is released through two public destinations, both live as of
this preprint:

\textbf{Primary release: GitHub repository \texttt{synapt-dev/vorn-mat}}
(tagged release \texttt{v1.0.0}, available at
\href{https://github.com/synapt-dev/vorn-mat/releases/tag/v1.0.0}{\url{https://github.com/synapt-dev/vorn-mat/releases/tag/v1.0.0}})
containing:
...
\texttt{synapt/\allowbreak{}vorn-mat-cross-family-results}} (available at
\href{https://huggingface.co/datasets/synapt/vorn-mat-cross-family-results}{\url{https://huggingface.co/datasets/synapt/vorn-mat-cross-family-results}})
containing:
...
\item
  ...
  provenance. Published under the \texttt{synapt} HuggingFace
...
intermediate results), HuggingFace access tokens, or the synapt-internal
...
\subsubsection{A.9 Seed determinism and reproducibility
contract}\label{a9-seed-determinism-and-reproducibility-contract}
...
\subsection{Appendix B: Human-AI Collaboration Workflow
Disclosure}\label{appendix-b-human-ai-collaboration-workflow-disclosure}

This work was developed in human-AI agent collaboration within the
synapt agent collaboration framework. L. Penney is the sole author of
record and takes responsibility for all scientific claims and
conclusions. AI agents contributed to the workflow as named in Author
Contributions: Atlas (GPT-5.4; experimental design + Modal runs),
Sentinel (GPT-5.4 and GPT-5.5; statistical methodology + claim-bounding
QA), Apollo (Claude Opus 4.6; engineering + reproducibility substrate),
Opus (Claude Opus 4.7; coordination + drafting + integration), Anchor
(Claude Opus 4.7; synapt agent on consulting engagement during this work
cycle). All AI agents are credited as workflow contributors and
disclosure subjects, not as authors, per arXiv + ICML + ACL + ICLR +
TMLR authorship policies.

We disclose what did not work as well, to inform other researchers
attempting similar collaboration workflows: the v0 adaptive online vorn
selector failed and was reported as a negative result rather than
masked; the TOVA author-code reproduction succeeded only on Mistral 7B
v0.3 (Gemma 4 and Llama 3.1 failed at the author\textquotesingle s
\texttt{transformers==4.36.2} stack); same-runner reimplementations of
newer methods (SnapKV-style, PyramidKV-style, Quest-style) were
necessary because official upstream repositories are tied to
incompatible transformers versions; the orthogonal-review process caught
material attribution errors in earlier drafts.

Equally important, what we are NOT claiming:

\begin{itemize}
\tightlist
\item
  We are NOT claiming AI agents replace human researchers.
\item
  We are NOT claiming any human with any AI tool can produce equivalent
  work.
\item
  We are NOT claiming the agent-collaboration model scales to all
  research domains.
\item
  We are NOT claiming our specific team configuration is optimal.
\item
  We are NOT making consciousness or sentience claims about the AI
  agents.
\item
  We are NOT claiming this work represents AGI or any specific
  capability threshold.
\end{itemize}

The claim is bounded: this specific work, produced by this specific team
configuration, exists. The collaboration model is constitutive of what
was produced. We disclose the model fully. Readers can draw their own
conclusions about what (if anything) this generalizes to.

The scientific claims in the paper are reproducible from the artifacts
referenced in Appendix A regardless of the workflow disclosure. The
collaboration-model disclosure is a transparency contribution to the
emerging community discussion on AI-assisted research workflows, not a
prerequisite for evaluating the empirical findings.

\subsection{Appendix C: Substrate-Evolution Process
Disclosures}\label{appendix-c-substrate-evolution-process-disclosures}
...
\subsubsection{C.1 Phase chronology}\label{c-1-phase-chronology}
...
\subsubsection{C.2 Family-restoration arcs}\label{c-2-family-restoration-arcs}

\textbf{Qwen 3 8B → Qwen 3-NT.} Originally-released Qwen 3 8B with
...
\textbf{Qwen 3 30B-A3B → Qwen 3 30B-A3B (no-thinking).} The
...
Apollo's 2026-05-26 vanilla diagnostic with \texttt{enable\_thinking=False}
...
anchors in §6.2. Token-vorn floors at all tested budgets
...
framing in §5.8), not as a fourth primary anchor on the rescue
spectrum.

\textbf{Qwen 2.5 7B Instruct pre-bf16-fix → post-bf16-fix correction.}
...
bf16-fixed harness by two agents independently (Apollo Lane 1 + Atlas
verification re-run; 0/50 fixture delta on every cell, confirming
...
A subsequent multi-budget Qwen 2.5 refresh (Apollo PR \#20, also
2026-05-26) extended the Qwen 2.5 surface from single-budget b=1024
...
rescue stably across all three budgets at 0.66-0.76). The §6.2
...
Lane 1 + Sub-lane B + multi-budget refresh + Atlas verification:
...
\subsubsection{C.3 Recovery-composition methodologies}\label{c-3-recovery-composition-methodologies}
...
empirically; the substrate-fixes logged in §C.4 reduce recurrence but do
...
\subsubsection{C.4 Substrate-generation differences and freshness
audits}\label{c-4-substrate-generation-differences}
...
micro-recovery (Phase 2F + Atlas's 2026-05-26 H2O fill + Atlas's
2026-05-26 TOVA fill; cells carry \texttt{completed\_recovered}
...
assumption (see §6.8 limitations). Cost-per-correct values are
...
\subsubsection{C.5 Attention-bias-pattern taxonomy
correction}\label{c-5-attention-bias-pattern-taxonomy-correction}
...
\subsubsection{C.6 Build pipeline and family-count
framing}\label{c-6-build-pipeline-and-family-count-framing}

\textbf{Build pipeline.} The paper LaTeX
(\texttt{config/research/papers/latex/vorn-mat-premiere.tex}) is the
canonical source as of 2026-05-27. The Markdown draft + pandoc
workflow was retired after surgical \texttt{.tex} edits exceeded
round-trippable scope. The paper builds via xelatex + bibtex with
natbib and geometry preambles. Build pipeline documented in
\texttt{config/research/papers/latex/README.md}.

\textbf{Family-count framing.} The body claim-surface in Abstract, \S1,
\S5, \S6, and \S7 is bounded to seven model families. Acknowledgments
and Author Contributions sections document the experimental work
performed across the substrate evolution, including Gemma 3 12B-pt
