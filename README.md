...
The Appendix A v1.1 totals (66 artifacts / 434 counted rows / 342 with observations / 21,600 per-fixture observations / $116.13 / 48.07h) are reproducible from this repository's `results/` directory by running:
...
The paired McNemar p-values cited in the paper are recoverable from the per-fixture `observations[]` arrays in the claim-bearing result rows. 342 of the 434 counted rows carry observations. See Appendix A for the counting contract. For example, the Llama 3.1 vorn cross-task headline cell (sentence-vorn 92/200 versus token-vorn 52/200 at b=1024 on qa_2_4k, paired exact McNemar `p = 1.03e-08`) traces to `results/token-vorn-qa2-cross-task-2026-05-20.json`.
